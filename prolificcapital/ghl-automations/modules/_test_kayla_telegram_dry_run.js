...
function realOpp(overrides = {}) { return opp({ opportunityId: 'realOpp123456789', contactId: 'realContact123456', propertyAddress: '123 Real St', phone: '+15555550123', raw: { locationId: '61XPzSqRy7UKMwW9DeB8', pipelineId: 'nSf3NXYVkt8X4PgW9aZ3', propertyFingerprint: 'fp-real' }, ...overrides }); }
const fixtures = [
...
await test('61 LIVE_ALLOWED cannot be entered', () => assert.throws(() => dry.setKillSwitch('LIVE_ALLOWED', ctx(400)), /UNAVAILABLE/));
...
await test('82 unknown timezone blocks integrated preview', () => { const r = tg.handleKaylaOutreachCommand(ctx(100), 'canary preview three', { opportunities: [realOpp()], now: new Date('2026-07-31T16:00:00Z'), timeZone: '' }); assert.ok(r.reply.includes('UNKNOWN_TIMEZONE_BLOCKS_CANARY')); });
...
await test('84 before 10 blocks integrated preview', () => { const r = tg.handleKaylaOutreachCommand(ctx(100), 'canary preview three', { opportunities: [realOpp()], now: new Date('2026-07-31T13:00:00Z'), timeZone: 'America/New_York' }); assert.ok(r.reply.includes('OUTSIDE_LOCAL_CANARY_WINDOW')); });
await test('85 after 6 blocks integrated preview', () => { const r = tg.handleKaylaOutreachCommand(ctx(100), 'canary preview three', { opportunities: [realOpp()], now: new Date('2026-07-31T22:00:00Z'), timeZone: 'America/New_York' }); assert.ok(r.reply.includes('OUTSIDE_LOCAL_CANARY_WINDOW')); });
await test('86 exact 10am boundary is sendable', () => { const r = tg.canaryPreviewForIntent({ count: 1 }, ctx(100), { opportunities: [realOpp()], now: new Date('2026-07-31T14:00:00Z'), timeZone: 'America/New_York' }); assert.ok(r.session.selectedRecords[0].ghlGuard.localTimeWindowStatus.passed); });
await test('87 exact 6pm boundary is blocked', () => { const r = tg.canaryPreviewForIntent({ count: 1 }, ctx(100), { opportunities: [realOpp()], now: new Date('2026-07-31T22:00:00Z'), timeZone: 'America/New_York' }); assert.strictEqual(r.session.selectedRecords[0].ghlGuard.localTimeWindowStatus.passed, false); });
await test('88 safe three-record preview passes GHL guard except isolation pending', () => { const records = [realOpp({ opportunityId: 'realOppA123456', contactId: 'realContactA123456', propertyAddress: 'A' }), realOpp({ opportunityId: 'realOppB123456', contactId: 'realContactB123456', propertyAddress: 'B' }), realOpp({ opportunityId: 'realOppC123456', contactId: 'realContactC123456', propertyAddress: 'C' })]; const r = tg.canaryPreviewForIntent({ count: 3 }, ctx(100), { opportunities: records, now: new Date('2026-07-31T16:00:00Z'), timeZone: 'America/New_York' }); assert.ok(r.session.selectedRecords.every(item => item.ghlGuard.stageMovementCapability.status === 'STAGE_MOVEMENT_DISABLED_WORKFLOW_ISOLATION_PENDING')); });
