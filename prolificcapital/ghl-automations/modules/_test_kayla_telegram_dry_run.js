...
await test('85 after 6 blocks integrated preview', () => { const r = tg.handleKaylaOutreachCommand(ctx(100), 'canary preview three', { opportunities: [realOpp()], now: new Date('2026-08-01T00:00:00Z'), timeZone: 'America/New_York' }); assert.ok(r.reply.includes('OUTSIDE_LOCAL_CANARY_WINDOW')); });
await test('86 exact 10am boundary is sendable', () => { const r = tg.canaryPreviewForIntent({ count: 1 }, ctx(100), { opportunities: [realOpp()], now: new Date('2026-07-31T15:00:00Z'), timeZone: 'America/New_York' }); assert.ok(r.session.selectedRecords[0].ghlGuard.localTimeWindowStatus.passed); });
await test('87 exact 6pm boundary is blocked', () => { const r = tg.canaryPreviewForIntent({ count: 1 }, ctx(100), { opportunities: [realOpp()], now: new Date('2026-07-31T23:00:00Z'), timeZone: 'America/New_York' }); assert.strictEqual(r.session.selectedRecords[0].ghlGuard.localTimeWindowStatus.passed, false); });
