...
await test('4 DNC tag produces BLOCKED state', () => assert.equal(guards.evaluateGhlComplianceLocks(opp({ tags: ['DNC'] })).checks.dnc, 'BLOCKED'));
await test('5 wrong number tag produces BLOCKED state', () => assert.equal(guards.evaluateGhlComplianceLocks(opp({ tags: ['wrong-number'] })).checks.wrongNumber, 'BLOCKED'));
await test('6 pending reply tag produces BLOCKED state', () => assert.equal(guards.evaluateGhlComplianceLocks(opp({ tags: ['pending reply'] })).checks.pendingReply, 'BLOCKED'));
...
await test('16 fail-closed compliance blocks plan with unknown states', () => { const result = guards.validateGhlCanaryPlan({ records: [opp({ opportunityId: 'a12345678', contactId: 'c12345678', propertyAddress: 'A' }), opp({ opportunityId: 'b12345678', contactId: 'd12345678', propertyAddress: 'B' }), opp({ opportunityId: 'e12345678', contactId: 'f12345678', propertyAddress: 'C' })], timeZone: 'America/New_York', now: new Date('2026-07-31T16:00:00Z'), killSwitchState: 'CANARY_ALLOWED' }); assert.strictEqual(result.ok, false); });
...
await test('20 MANUAL_LIVE_ALLOWED is not a valid state', () => assert.ok(!guards.LIVE_KILL_SWITCH_STATES.includes('MANUAL_LIVE_ALLOWED')));
